...
void simulation(std::vector<double> *u_1, std::vector<double> *beta, std::vector<double> *coeff, double const& dx, double const& dt, int const& eqref, int const& ucase, int const& Npos, double const& tfinal, double const& omega, double const& A, boundary_condition const& left_bc, double const& leftboundaryvalue, boundary_condition const& right_bc, double const& rightboundaryvalue, mode const& sortie, std::ofstream& w_ofs, std::ofstream& energy_ofs, std::ofstream& maxenergy_ofs, bool parrallel = true)
{
    ...
    do
    {
          ...
          if(eqref == 1)
          {
              #pragma omp parallel for if(Npos > 1000 && parrallel)
